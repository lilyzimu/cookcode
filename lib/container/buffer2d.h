...
class buffer2d {
 public:
  ...
  const_reference operator()(const vec2d<size_type> &pos) const { return _at(pos); }
  reference       operator()(const vec2d<size_type> &pos)       { return _at(pos); }
 ...
 protected:
  ...
  const_reference _at(const vec2d<size_type>& pos) const { return _at(pos.x,pos.y); }
  reference       _at(const vec2d<size_type>& pos)       { return _at(pos.x,pos.y); }
