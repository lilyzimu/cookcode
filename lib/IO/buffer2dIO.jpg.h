...
namespace io {
  namespace jpg {
    ...
    template<typename Buffer, typename C>
      void _export(const string& filename, const Buffer& buf, float pad=0.0f, float compressionQuality=0.95f) { throw unsupportedFormat(); }
    ...
    ////////////////
    template<typename Buffer, typename C>
      void _export(const string& filename, const Buffer& buf, float pad=0.0f, float compressionQuality=0.95f)
    {
      // open file
	...
	// convert
	for(unsigned int i = 0; i < buf.width(); i++, itr++)
	{
	  C wrapper(*itr);
	...
	// convert
	for(unsigned int i = 0; i < result.width(); i++, itr++)
	{
	  unsigned int count=0;
	  C wrapper(*itr);
	  for(typename C::iterator typeItr = wrapper.begin(); typeItr != wrapper.end(); typeItr++, count++)
	    if(count < 3) *typeItr = (*buffer)[i*3+count] / 255.0f;
	    else *typeItr = pad;
	}
