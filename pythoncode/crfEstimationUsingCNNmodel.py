...
plotoutpath = '/big/temp/hanli/data/moredata/zztestscene/resnet18/setOverExpoToWhite/plotoutTestScene/'
curveWithImagepath = '/big/temp/hanli/data/moredata/zztestscene/resnet18/setOverExpoToWhite/curveWithTestImagePlotOut/'
...
for index in range(0, selectPlotOutTestpngNumber):
    ...
    sep = ' '
    cropcurvename =  (str(inputdatastr[index]).split(sep)[0]).split('pcacurveExpo/')[1]
#    cropcurvename = 'scene1smallcurve1CropWin0.png'
    ...
    f = os.popen('/home/hanli/caffe/build/tools/caffe test -model /home/hanli/caffe/examples/crf6resnet/deploy.prototxt -weights /big/temp/hanli/data/moredata/datasnapshotcopy/snapshot/_iter_200000.caffemodel -iterations 1') 
    now = f.read()
    ...
    with h5py.File('/big/temp/hanli/data/moredata/datasnapshotcopy/snapshot/output.h5','r') as hf:
        ...
        prob = hf.get('label')
        np_prob = np.array(prob).reshape(201)
    ...
    for estTopN in range(0,TopNumber):
        plt.plot(rawdata, estpngdata[:,estTopN], color=plotcolors[estTopN], linewidth=1.5, linestyle="--", label='estTop' + str(estTopN) + 'curve: ' + str(int((output_curveIndex[estTopN]))))
