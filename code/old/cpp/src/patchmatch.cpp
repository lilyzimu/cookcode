...
void reconstructa(BITMAP *a, BITMAP *b, BITMAP *ann, BITMAP *annd, string filename)
{
  int aew = a->w - patch_w+1, aeh = a->h - patch_w + 1;       /* Effective width and height (possible upper left corners of patches). */
  int bew = b->w - patch_w+1, beh = b->h - patch_w + 1;
  // save the reconstructed a.png according to nnf. 
  image imgafromnnfb(aew, aeh);  
  for (int ay = 0; ay < aeh; ay++) {
    for (int ax = 0; ax < aew; ax++) {
      int v = (*ann)[ay][ax];
      int xbest = INT_TO_X(v), ybest = INT_TO_Y(v);
      int dbest = (*annd)[ay][ax];
      int bc = (*b)[ybest][xbest];
      double range = 256.0;
      imgafromnnfb(ax,ay).r = ( bc&255 )/range;
      imgafromnnfb(ax,ay).g = ( (bc>>8)&255 )/range;
      imgafromnnfb(ax,ay).b = ( (bc>>16)&255 )/range;
    }
  }
  io::exportImage(filename, imgafromnnfb);
  
}
int main(int argc, char *argv[]) {
  std::cout<< "loading input images: " << std::endl;
  std::string filenamea = "../../data/wren1.png";
  std::string filenameb = "../../data/wren2.png";
  std::string saveann = "../../data/ann.png";
  std::string saveannd = "../../data/annd.png";
  std::string finalimg = "../../data/finala.png";
  const char* namea = filenamea.c_str();
  const char * nameb = filenameb.c_str();
  const char * annpath = saveann.c_str();
  const char * anndpath = saveannd.c_str(); 
  BITMAP *a = load_bitmap(namea);
  BITMAP *b = load_bitmap(nameb);
  BITMAP *ann = NULL, *annd = NULL;
  printf("Running PatchMatch\n");
  patchmatch(a, b, ann, annd);
  printf("Saving output images\n");
  save_bitmap(ann, annpath);
  save_bitmap(annd, anndpath);
  reconstructa(a, b, ann, annd, finalimg);
  return 0;
}
